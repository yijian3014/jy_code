...
namespace ntkoofficedemo_vs2008_
{
    public class ftp_jy
    {
        ...
        { ///从指定路径下载指定文件
           ConfigurationManager.AppSettings["file_jzjd"] = "";
                ConfigurationManager.AppSettings["file_jzjd"] = "开始下载文档。。";
            ...
            {
                ConfigurationManager.AppSettings["file_jzjd"] = "下载出错：" + ex.Message.ToString();
                return false;
            }
        ...
        {
            ConfigurationManager.AppSettings["file_jzjd"] = "";
            ConfigurationManager.AppSettings["file_jzjd"] = "扫描远程FTP文件列表。。。";
            ...
            catch (Exception ex)
            {
                ...
                {
                    ConfigurationManager.AppSettings["file_jzjd"] = "扫描远程FTP文件出错：" + ex.Message.ToString();
                }
                ConfigurationManager.AppSettings["file_jzjd"] = "扫描远程FTP文件出错：" + ex.Message.ToString();
        ...
        {
            ConfigurationManager.AppSettings["file_jzjd"] = "";
            ConfigurationManager.AppSettings["file_jzjd"] = "正在分析文件是否存在于缓存。。。";
            ...
            try
            {
                if (!File.Exists(abs_path + RemoteFileName))
                    //存在文件，数据库更新访问信息，不存在则从远程下载文档，更新缓存信息，并整理缓存！
                  {
                    ...
                    foreach (string str in fileList)
                    {
                        ...
                        if (str.Trim() == RemoteFileName.Trim())
                        {
                            ...
                            //inst_file_str字符串操作缓存管理表，如果存在记录则更新，如果不存在，则插入新记录
                            //下面是下载动作
                ...
                else
                {
                    ...
                    //upd_file_str字符串操作缓存管理表，如果存在记录则更新，如果不存在，则插入新记录
            ...
            {
                //没法将服务器错误带回前台，待修正
                ConfigurationManager.AppSettings["file_jzjd"] = "缓存管理出错：" + ex.Message.ToString();
                return false;
            }
            }
